...
class LinkedList{
    ...
    removeAt(index){
        ...
        LinkedList.validateIndex(index);

        if(index === 0) return this.removeHead();
        if(index == this.size || index === this.size - 1) return this.removeTail();
    ...
    listSize(){
        console.log(`The Linked List has ${this.size} elements`);
        return this.size;
    };

    printHeadAndTail(){
        return console.log(`The head of the linked list is "${this.head.data}" and the tail of the linked list is "${this.tail.data}"`);
    };
    ...
    static checkDataType(data){
        if(typeof data !== 'string' && typeof data !== 'number') throw new TypeError('The Linked List only accepts numbers or strings.');    
    };

    static validateIndex(index, size){
        if(typeof index !== "number") throw new TypeError("Index must be a number.");
        if(index < 0 || index > size) throw new RangeError(`The index ${index} is out of bounds.`);
